...
contract VaultAccountingLibHarness {
    ...
    function computePreBatch(uint256 navPrev, uint256 sharesPrev, int256 pnl, uint256 fees, uint256 grant)
        external
        pure
        returns (uint256 navPre, uint256 batchPrice)
    {
        VaultAccountingLib.PreBatchInputs memory inputs = VaultAccountingLib.PreBatchInputs({
            navPrev: navPrev, sharesPrev: sharesPrev, pnl: pnl, fees: fees, grant: grant
        });
    ...
    function computePostBatchState(uint256 nav, uint256 shares, uint256 previousPeak)
        external
        pure
        returns (uint256 navOut, uint256 sharesOut, uint256 price, uint256 pricePeak, uint256 drawdown)
    {
        VaultAccountingLib.PostBatchState memory state =
            VaultAccountingLib.computePostBatchState(nav, shares, previousPeak);
