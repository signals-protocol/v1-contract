...
export async function upgradeAction(env: Environment) {
  ...
  const newCoreImpl = await upgrades.erc1967.getImplementationAddress(
    await upgradedCore.getAddress(),
  );
  ...
  const newPositionImpl = await upgrades.erc1967.getImplementationAddress(
    await upgradedPosition.getAddress(),
  );

  let newLpShareImpl: string | undefined;
  if (lpShareProxyAddr) {
    const lpShareFactory = await ethers.getContractFactory('SignalsLPShare');
    const upgradedLpShare = await upgrades.upgradeProxy(
      lpShareProxyAddr,
      lpShareFactory,
      { kind: 'uups' },
    );
    await upgradedLpShare.waitForDeployment();
    newLpShareImpl = await upgrades.erc1967.getImplementationAddress(
      await upgradedLpShare.getAddress(),
    );
  }
