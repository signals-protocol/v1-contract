...
function parseModulesToUpdate(): Set<ModuleKey> {
  ...
  for (const entry of raw.split(',')) {
    ...
    if (!moduleName) {
      throw new Error(
        `Unknown module "${entry}". Expected trade, lifecycle, oracle, risk, vault.`,
      );
    }
...
export async function updateModulesAction(env: Environment) {
  ...
  if (updateTrade) {
    if (!lazyAddress)
      throw new Error(
        'LazyMulSegmentTree is required for TradeModule deployment',
      );
  ...
  if (updateLifecycle) {
    if (!lazyAddress)
      throw new Error(
        'LazyMulSegmentTree is required for MarketLifecycleModule deployment',
      );
    ...
    throw new Error(
      'Missing MarketLifecycleModule address in environment file',
    );
  }
  ...
  let vaultModuleAddr =
    envData.contracts.LPVaultModule ??
    envData.contracts.VaultModule ??
    ethers.ZeroAddress;
  if (updateVault) {
