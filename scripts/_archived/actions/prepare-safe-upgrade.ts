...
function requireAddress(value: string | undefined, label: string): string {
  if (!value) {
    throw new Error(`Missing ${label} in environment file`);
  }
  return value;
}
...
async function main() {
  ...
  const coreProxy = requireAddress(
    envData.contracts.SignalsCoreProxy,
    'SignalsCoreProxy',
  );
  const existingTrade = requireAddress(
    envData.contracts.TradeModule,
    'TradeModule',
  );
  const existingLifecycle = requireAddress(
    envData.contracts.MarketLifecycleModule,
    'MarketLifecycleModule',
  );
  const existingOracle = requireAddress(
    envData.contracts.OracleModule,
    'OracleModule',
  );
  const existingRisk = requireAddress(
  ...
  const existingLazy = requireAddress(
    envData.contracts.LazyMulSegmentTree,
    'LazyMulSegmentTree',
  );

  console.log(
    `[prepare-safe-upgrade] env=${env} network=${network.name} chainId=${chainId.toString()}`,
  );
  console.log(`[prepare-safe-upgrade] deployer=${deployer.address}`);
  console.log(
    `[prepare-safe-upgrade] modules=${Array.from(modulesToDeploy).join(',')}`,
  );
  ...
  const upgradeToAndCallCalldata = coreIface.encodeFunctionData(
    'upgradeToAndCall',
    [newCoreImpl, setModulesCalldata],
  );
  ...
  writeCopyFile(
    copyDir,
    '06-arg-data-setModulesCalldata.txt',
    setModulesCalldata,
  );
  writeCopyFile(
    copyDir,
    '07-calldata-upgradeToAndCall.txt',
  ...
  console.log('');
  console.log('[Deployed contracts for env update after Safe execution]');
  console.log(`SignalsCoreImplementation: ${newCoreImpl}`);
  console.log(`LazyMulSegmentTree: ${lazyAddress}`);
  console.log(`TradeModule: ${tradeModule}`);
  console.log(`MarketLifecycleModule: ${lifecycleModule}`);
  console.log(`OracleModule: ${oracleModule}`);
  console.log(`RiskModule: ${riskModule}`);
  console.log(`LPVaultModule: ${vaultModule}`);
  console.log('');
