...
async function main() {
  ...
  console.log(
    `[prepare-safe-position-upgrade] env=${env} chainId=${chainId.toString()}`,
  );
  ...
  console.log(
    `[prepare-safe-position-upgrade] new implementation deployed: ${newImpl}`,
  );

  const positionIface = (await ethers.getContractFactory('SignalsPosition'))
    .interface;
  ...
  const outPath = path.join(
    'releases',
    env,
    `${tag}-safe-position-upgrade-plan.json`,
  );
  const copyDir = path.join(
    'releases',
    env,
    `${tag}-safe-position-upgrade-copy`,
  );
