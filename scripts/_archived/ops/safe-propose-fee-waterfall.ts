...
const SET_FEE_WATERFALL_ABI = [
  {
    type: 'function' as const,
    name: 'setFeeWaterfallConfig',
    stateMutability: 'nonpayable' as const,
    inputs: [
      { name: 'rhoBS', type: 'uint256' },
      { name: 'phiLP', type: 'uint256' },
      { name: 'phiBS', type: 'uint256' },
      { name: 'phiTR', type: 'uint256' },
    ],
    outputs: [],
  },
] as const;
...
function outputPath(env: Environment) {
  ...
  const filename = suffix
    ? `${env}.${OUTPUT_PREFIX}.${suffix}.json`
    : `${env}.${OUTPUT_PREFIX}.json`;
...
async function main() {
  ...
  if (!rawEnv) {
    console.error(
      'Usage: ENV=dev|prod tsx scripts/ops/safe-propose-fee-waterfall.ts',
    );
    process.exit(1);
  }
  const env = normalizeEnvironment(rawEnv) as Environment;
  console.log(`[safe-propose] environment=${env}`);

  const chainId = BigInt(EXPECTED_CHAIN_IDS[env] ?? 0);
  if (chainId === 0n) {
  ...
  if (!coreAddress) {
    throw new Error(
      'Missing CORE_ADDRESS (or SignalsCoreProxy in environment file)',
    );
  }
  ...
  if (!safeAddress) {
    throw new Error(
      'Missing SAFE_ADDRESS (or owners.core in environment file)',
    );
  }
  ...
  if (phiSum !== WAD) {
    throw new Error(
      `phiLP+phiBS+phiTR must equal 1e18 (got ${phiSum.toString()})`,
    );
  }

  const data = encodeFunctionData({
    abi: SET_FEE_WATERFALL_ABI,
    functionName: 'setFeeWaterfallConfig',
    args: [rhoBS, phiLP, phiBS, phiTR],
  });
  ...
  const proposerKey =
    ...
    '';
  const rpcUrl = process.env.SAFE_RPC_URL?.trim() ?? CHAIN_RPC[env];
  ...
  if (shouldPropose) {
    try {
      if (!proposerKey) {
        throw new Error(
          'SAFE_PROPOSER_KEY or DEPLOYER_KEY is required to propose a Safe transaction',
        );
      }
      if (!rpcUrl) {
        throw new Error(
          'SAFE_RPC_URL is required when proposing to the Safe service',
        );
      }
      ...
      if (!isOwner) {
        throw new Error(
          `SAFE_PROPOSER_KEY is not a Safe owner (${proposer.address})`,
        );
      }
      ...
      if (gatewayUrl) {
        ...
        if (!res.ok) {
          throw new Error(
            `Gateway propose failed (${res.status}): ${text.slice(0, 200)}`,
          );
        }
        ...
        console.log(
          `[safe-propose] proposed via tx-service txHash=${safeTxHash}`,
        );
      }
    } catch (err) {
    ...
    console.log(
      '[safe-propose] SAFE_API_KEY/SAFE_TX_SERVICE_URL/SAFE_GATEWAY_URL not set; wrote Transaction Builder JSON only',
    );
  }
