...
async function main() {
  ...
  if (!paymentTokenAddress)
    throw new Error('Missing payment token in environment file');
  ...
  if (!isOwner) {
    ...
    console.log(
      '[run-cycle] deployer is operator; skipping owner-only config updates',
    );
  }
  ...
  if (isOwner) {
    await (
      await core.setRiskConfig(
        wad(CONFIG.risk.lambda),
        wad(CONFIG.risk.kDrawdown),
        CONFIG.risk.enforceAlpha,
      )
    ).wait();
    await (
  ...
  console.log(
    `[run-cycle] marketId=${marketId.toString()} created tx=${createTx.hash}`,
  );

  let remaining = numBins;
  while (remaining > 0) {
    const count =
      remaining > CONFIG.market.seedChunkSize
        ? CONFIG.market.seedChunkSize
        : remaining;
  ...
  const openTx = await core.openPosition(
    marketId,
    lowerTick,
    upperTick,
    quantity,
  ...
  console.log(
    `[run-cycle] positionId=${positionId.toString()} openTx=${openTx.hash}`,
  );
  ...
  if (isOwner) {
    const windowLagMax =
      submitWindowSec - CONFIG.settlementTiming.minWindowRemainingSec;
    const distanceLimit =
      maxSampleDistance > 0
        ? maxSampleDistance - CONFIG.settlementTiming.lagBufferSec
        : windowLagMax;
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      console.log(
        `[run-cycle] marketState=${state.toString()} attempt=${attempt + 1}`,
      );
      const finalizeTx = await core.finalizePrimarySettlement(marketId);
      await finalizeTx.wait();
      console.log(
        `[run-cycle] finalizePrimarySettlement tx=${finalizeTx.hash}`,
      );
      finalized = true;
      break;
    } catch (err) {
      ...
      console.log(
        `[run-cycle] finalize failed reason=${decoded.name ?? 'unknown'}`,
      );
      if (
      ...
      if (decoded.name === 'SettlementOracleCandidateMissing') {
        ...
        const retrySubmit = await wrapped.submitSettlementSample(
          marketId.toString(),
        );
  ...
  let calls = 0;
  while (
    !market.snapshotChunksDone &&
    calls < CONFIG.settlementTiming.maxChunkCalls
  ) {
    const chunkTx = await core.requestSettlementChunks(
      marketId,
      CONFIG.settlementTiming.maxChunksPerTx,
    );
    await chunkTx.wait();
    calls += 1;
  ...
  console.log(
    `[run-cycle] settlement chunks done=${market.snapshotChunksDone}`,
  );
  ...
  if (batchId === currentBatchId + 1n && total > 0n && resolved === total) {
    ...
    await batchTx.wait();
    console.log(
      `[run-cycle] processDailyBatch batchId=${batchId.toString()} tx=${batchTx.hash}`,
    );
  } else {
    console.warn(
  ...
  try {
    ...
    await claimTx.wait();
    console.log(
      `[run-cycle] claimPayout positionId=${positionId.toString()} tx=${claimTx.hash}`,
    );
  } catch (err) {
    const decoded = await decodeRevert(err);
    if (decoded.name === 'ClaimTooEarly' && decoded.args?.length) {
      ...
      console.log(
        `[run-cycle] ClaimTooEarly; waiting ${waitMore}s and retrying`,
      );
  ...
  console.log(
    `[run-cycle] finished. deployer balance=${balance.toString()} (6 decimals)`,
  );
}
