...
library LazyMulSegmentTreeNoLink {
    ...
    function applyRangeFactor(Tree storage tree, uint32 lo, uint32 hi, uint256 factor) internal {
    ...
    function getRangeSum(Tree storage tree, uint32 lo, uint32 hi) internal view returns (uint256 sum) {
        if (tree.size == 0) revert SE.TreeNotInitialized();
        if (lo > hi) revert SE.InvalidRange(lo, hi);
        if (hi >= tree.size) revert SE.IndexOutOfBounds(hi, tree.size);
    ...
    function propagateLazy(Tree storage tree, uint32 lo, uint32 hi) internal returns (uint256 sum) {
        if (tree.size == 0) revert SE.TreeNotInitialized();
        if (lo > hi) revert SE.InvalidRange(lo, hi);
        if (hi >= tree.size) revert SE.IndexOutOfBounds(hi, tree.size);
