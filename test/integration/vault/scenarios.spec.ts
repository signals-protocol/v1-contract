...
describe("LP Vault Scenarios", () => {
  async function deployVaultFixture() {
    ...
    // FeeWaterfall config (pdd is already set via setRiskConfig)
  ...
  describe("Scenario: Happy Path 3-Day", () => {
    ...
    it("completes full 3-day deposit/trade/withdraw cycle", async () => {
      ...
      await advancePastBatchEnd(day3);
      ...
      await proxy.harnessRecordPnl(day4, 0n, 0n, ethers.parseEther("500"));
      await advancePastBatchEnd(day4);
  ...
  describe("Scenario: Bank Run", () => {
    ...
    it("handles simultaneous large withdrawal requests", async () => {
      ...
      await proxy.harnessRecordPnl(day3, 0n, 0n, ethers.parseEther("500"));
      await advancePastBatchEnd(day3);
      await proxy.processDailyBatch(day3);
